 ...
 */
__global__ void convolve(__constant__ unsigned char *source, int width, int height, int paddingX, int paddingY, ssize_t kOffset, int kWidth, int kHeight, unsigned char *destination)
{
...
int main (int argc, char** argv)
{
    ...
    unsigned char *d_org_img, *d_edge_img, *d_blurred_img;
    cudaMalloc((void **) &deviceGradientX, frame.size().width * frame.size().height);
    cudaMalloc((void **) &deviceGradientY, frame.size().width * frame.size().height);
    cudaMalloc((void **) &d_org_img, 2*frame.size().width * frame.size().height);
    cudaMalloc((void **) &d_edge_img, frame.size().width * frame.size().height);
    cudaMalloc((void **) &d_blurred_img, frame.size().width * frame.size().height);
    ...
    for (string filename:fn)
    {
        ...
        cudaEventRecord(start);
        {
            ...
            cudaMemcpy(d_org_img, frame.data, 2*frame.size().width * frame.size().height, cudaMemcpyHostToDevice);
            // cudaMemcpy(h_source.data,d_org_img, 2*frame.size().width * frame.size().height, cudaMemcpyDeviceToHost);
            // frame.copyTo(source);
            ...
            // convolve<<<cblocks,cthreads>>>(d_blurred_img, frame.size().width, frame.size().height, 2, 2, sobelGradientXOffset, 3, 3, d_blurred_img);
            // convolve<<<cblocks,cthreads>>>(d_blurred_img, frame.size().width, frame.size().height, 2, 2, sobelGradientYOffset, 3, 3, d_blurred_img);
            // pythagoras<<<pblocks,pthreads>>>(deviceGradientX, deviceGradientY, d_edge_img);

            cudaMemcpy(h_source.data,d_org_img, 2*frame.size().width * frame.size().height, cudaMemcpyDeviceToHost);
            cudaMemcpy(blurred.data,d_blurred_img, frame.size().width * frame.size().height, cudaMemcpyDeviceToHost);
            cudaMemcpy(edges.data,d_edge_img, frame.size().width * frame.size().height, cudaMemcpyDeviceToHost);
