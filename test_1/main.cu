...
int main (int argc, char** argv)
{
    ...
    cudaMalloc(&d_org_img, frame.rows * frame.cols* sizeof(unsigned char));
    ...
    for (string filename:fn)
    {
        ...
        cudaEventRecord(start);
        {
            ...
            cudaMemcpy(d_org_img, frame.data, frame.rows * frame.cols* sizeof(unsigned char), cudaMemcpyHostToDevice);
            cudaMemcpy(h_source.data,d_org_img, frame.rows * frame.cols* sizeof(unsigned char), cudaMemcpyDeviceToHost);
            // frame.copyTo(source);
