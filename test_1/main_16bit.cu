 ...
 */
__global__ void convolve(unsigned short *source, int width, int height, int paddingX, int paddingY, ssize_t kOffset, int kWidth, int kHeight, unsigned short *destination)
{
    ...
    // Average the sum
    destination[(y * width) + x] = (unsigned short) sum;
}

// converts the pythagoran theorem along a vector on the GPU
__global__ void pythagoras(unsigned short *a, unsigned short *b, unsigned short *c)
{
    ...
    c[idx] = (unsigned short) sqrtf(af*af + bf*bf);
}
...
int main (int argc, char** argv)
{
    ...
    // camera >> frame;
    Mat h_source;
    Mat frame = imread("images/frameIndex_0_2019.png", -1);
    if (frame.empty()) 
    {
    ...
    unsigned short *deviceGradientX, *deviceGradientY;
    unsigned short *d_org_img, *d_edge_img, *d_blurred_img;
    ...
    cudaMalloc(&d_org_img, frame.rows * frame.cols* sizeof(unsigned short));
    cudaMalloc(&d_edge_img, frame.size().width * frame.size().height);
    cudaMalloc(&d_blurred_img, frame.size().width * frame.size().height);
    ...
    string path = "/home/delta/CUDA_git/dataset_thermal/Denmark/Copenhagen/20191126_095918_done/16BitTransformed/*.png";
    ...
    for (string filename:fn)
    {
        ...
        cout << "rows:" << frame.rows << "cols:" << frame.cols << "steps:" << frame.step << endl;
        ...
        cudaEventRecord(start);
        {
            ...
            // h_source.data = frame.data;
            cudaMemcpy(d_org_img, frame.data, frame.rows * frame.cols* sizeof(unsigned short), cudaMemcpyHostToDevice);
            // cudaMemcpy(h_source.data,d_org_img, frame.rows * frame.cols* sizeof(unsigned short), cudaMemcpyDeviceToHost);
            // frame.copyTo(source);

            //             // Perform the gaussian blur (first kernel in store @ 0)
            convolve<<<cblocks,cthreads>>>(d_org_img, frame.size().width, frame.size().height, 0, 0, gaussianKernel5x5Offset, 5, 5, d_blurred_img);
            ...
            convolve<<<cblocks,cthreads>>>(d_blurred_img, frame.size().width, frame.size().height, 2, 2, sobelGradientXOffset, 3, 3, deviceGradientX);
            convolve<<<cblocks,cthreads>>>(d_blurred_img, frame.size().width, frame.size().height, 2, 2, sobelGradientYOffset, 3, 3, deviceGradientY);
            cudaThreadSynchronize();

            pythagoras<<<pblocks,pthreads>>>(deviceGradientX, deviceGradientY, d_edge_img);

            cudaMemcpy(h_source.data,d_org_img, frame.rows * frame.cols* sizeof(unsigned short), cudaMemcpyDeviceToHost);
            // cudaMemcpy(blurred.data,d_blurred_img, frame.rows * frame.cols* sizeof(unsigned short), cudaMemcpyDeviceToHost);
            // cudaMemcpy(edges.data,d_edge_img, frame.rows * frame.cols* sizeof(unsigned short), cudaMemcpyDeviceToHost);
        ...
        cv::imshow("Greyscale", h_source);
        cv::imshow("Blurred", blurred);
        cv::imshow("Sobel", edges);

        // Spin
        // waitKey(0);
        if(cv::waitKey(40 - ms) == 27) break;
    }
